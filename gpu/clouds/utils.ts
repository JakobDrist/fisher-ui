...
import {
  CLOUD_AMPLITUDE, CLOUD_BRIGHT, CLOUD_COVERAGE, CLOUD_DARK, CLOUD_FREQUENCY,
  FBM_LACUNARITY, FBM_OCTAVES, FBM_PERSISTENCE, LIGHT_ABSORPTION,
  NOISE_TEXTURE_SIZE, NOISE_Z_OFFSET, SKY_AMBIENT, SUN_BRIGHTNESS, SUN_COLOR,
} from "./consts";
import { cloudsLayout } from "./types";
...
export const raymarch = tgpu.fn([d.vec3f, d.vec3f, d.vec3f], d.vec4f)(
  (rayOrigin, rayDir, sunDir) => {
    ...
    for (let i = 0; i < maxSteps; i++) {
      ...
      if (cloudDensity > 0.0) {
        const shadowDensity = sampleDensityCheap(samplePos + sunDir);
        ...
        const contrib = d.vec4f(color * light, 1) * cloudDensity * (LIGHT_ABSORPTION - accum.a);
        accum += contrib;
        if (accum.a >= LIGHT_ABSORPTION - 0.001) break;
      }
      dist += stepSize;
    }
    return accum;
...
function fbm(pos: d.v3f): number {
  ...
  for (const i of tgpu.unroll(std.range(FBM_OCTAVES))) {
    sum += noise3d(pos * (CLOUD_FREQUENCY * FBM_LACUNARITY ** i)) *
...
const noise3d = tgpu.fn([d.vec3f], d.f32)((pos) => {
  ...
  const texCoord1 = std.fract((idx.xy + frac.xy + NOISE_Z_OFFSET * (idx.z + 1)) / NOISE_TEXTURE_SIZE);
  const val0 = std.textureSampleLevel(cloudsLayout.$.noiseTexture, cloudsLayout.$.sampler, texCoord0, 0).x;
  const val1 = std.textureSampleLevel(cloudsLayout.$.noiseTexture, cloudsLayout.$.sampler, texCoord1, 0).x;
