...
export function Field({
  ...
  return (
    <div className="relative rounded-2xl border border-zinc-200 bg-zinc-50/80 p-3.5 shadow-[0_2px_8px_rgba(15,23,42,0.04)] dark:border-border/50 dark:bg-background/40 dark:shadow-none">
      <label
        htmlFor={id}
      ...
      <div className="flex items-center justify-between gap-3">
        ...
        <button
          ...
          className="group inline-flex h-10 items-center gap-2 rounded-full border border-zinc-300 bg-white pl-1 pr-2.5 text-sm font-semibold text-foreground shadow-sm transition-transform hover:border-zinc-400 active:scale-[0.97] dark:border-border dark:bg-card dark:shadow-none"
        >
      ...
      <div className="mt-2 flex items-center justify-between text-[11px] text-muted-foreground">
        ...
        {side === "from" && token.balance ? (
          <button
            ...
            className="rounded px-1.5 py-0.5 text-[10px] font-semibold uppercase tracking-wider text-muted-foreground hover:bg-zinc-200 hover:text-foreground dark:hover:bg-primary/5"
          >
            Max
          </button>
        ) : null}
