...
export class Slider {
  ...
  constructor(root: TgpuRoot, start: d.v2f, end: d.v2f, numPoints: number, yOffset = 0) {
    ...
    this.#computeBezierPipeline = this.#root.createGuardedComputePipeline((x, y) => {
      ...
      const pixelUV = (d.vec2f(x, y) + 0.5) / d.vec2f(size);
      ...
      for (let i = 0; i < pointsView.$.length - 1; i++) {
        ...
        xPlusDist = std.min(xPlusDist, sdBezier(sliderPos + xOffset, A, C, B));
        xMinusDist = std.min(xMinusDist, sdBezier(sliderPos - xOffset, A, C, B));
        yPlusDist = std.min(yPlusDist, sdBezier(sliderPos + yOffset, A, C, B));
        yMinusDist = std.min(yMinusDist, sdBezier(sliderPos - yOffset, A, C, B));
      }
