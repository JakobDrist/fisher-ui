...
function CodeTabs({
  ...
  return (
    <Tabs
      ...
      className={cn(
        'w-full gap-0 overflow-hidden rounded-xl border border-border bg-background shadow-sm',
        className,
      )}
      {...props}
      value={selectedCode}
    ...
    >
      <TabsHighlight className="absolute inset-0 z-0 rounded-none bg-transparent shadow-none after:absolute after:inset-x-0 after:bottom-0 after:h-0.5 after:rounded-t-full after:bg-foreground/70 after:content-['']">
        <TabsList
          data-slot="install-tabs-list"
          className="relative flex h-10 w-full items-center justify-between rounded-none border-b border-border/70 bg-muted/55 px-4 py-0 text-current"
        >
