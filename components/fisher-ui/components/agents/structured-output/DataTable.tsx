...
// Official brand marks from Simple Icons (viewBox 0 0 24 24).
const OPENAI =
...
function ModelIcon({ model }: { model: string }) {
  ...
  return (
    <span
      ...
      style={{ "--brand-color": brand.bg } as CSSProperties}
    >
...
export function DataTable({ rows = DEFAULT_ROWS }: { rows?: DataTableRow[] }) {
  return (
    <div aria-label="AI model pricing" className={styles.table} role="table">
      <div className={styles.tableHead} role="row">
        {["Model", "Context", "$/1M in"].map((heading) => (
          <div className={styles.tableCell} key={heading} role="columnheader">
            {heading}
          </div>
        ))}
      </div>
      ...
      <div className={styles.tableBody} role="rowgroup">
        {rows.map((row) => (
          <div className={styles.tableRow} key={row.model} role="row">
            <div className={styles.tableCell} role="cell">
              <span className={styles.modelCell}>
                <ModelIcon model={row.model} />
                <span className={styles.tableCellText}>{row.model}</span>
              </span>
            </div>
            <div className={styles.tableCell} role="cell">
              {row.context}
