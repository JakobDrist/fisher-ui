...
async function captureHtmlAsCanvas(content: HTMLElement) {
  ...
  if (!context) throw new Error("Could not create the particle capture canvas.");
  ...
  content.querySelectorAll<HTMLElement>("*").forEach((node) => {
    ...
    if (parseFloat(styles.borderTopWidth) > 0) {
      context.strokeStyle = "rgba(248, 250, 252, 0.16)";
      context.lineWidth = Math.min(parseFloat(styles.borderTopWidth), 1);
    ...
    const fontSize = Math.max(parseFloat(styles.fontSize) || 16, 1);
    const lineHeight = Math.max(parseFloat(styles.lineHeight) || fontSize * 1.25, 1);
...
export function createParticleScroll(
  ...
  let fallbackCaptureQueued = false;
  ...
  // Canvas UI's native path uses the experimental HTML-in-canvas API. This
  // browser-native SVG capture keeps the WebGL effect available elsewhere
  // without relying on a CSS parser that cannot read modern lab()/oklab().
  function queueFallbackCapture() {
    if (htmlInCanvas || fallbackCaptureQueued) return;
    fallbackCaptureQueued = true;

    requestAnimationFrame(async () => {
      fallbackCaptureQueued = false;
      try {
        fallbackSource = await captureHtmlAsCanvas(content);
        contentDirty = true;
        wake();
      } catch (error) {
        console.error("ParticleScroll fallback capture failed", error);
        // Leave the accessible HTML fallback visible if a capture is blocked.
      }
    });
  }
  ...
  function uploadContent() {
    const capture = htmlInCanvas ? source : fallbackSource;
    if (!capture || !contentDirty) return;
    contentDirty = false;
    introReady = true;
    syncBgColor();
    gl!.bindTexture(gl!.TEXTURE_2D, contentTexture);
  ...
  function render(dt: number) {
    uploadContent();
    const captureAvailable = htmlInCanvas || fallbackSource !== null;
    ...
    gl!.uniform1f(base.uniforms.uCover, captureAvailable ? 1 : 0);
    ...
    if (!captureAvailable || rowsAssembled) return;
    gl!.enable(gl!.BLEND);
