...
function Code({ className, code, ...props }: CodeProps) {
  return (
    <CodeProvider value={{ code }}>
      <div
        className={cn(
          'relative flex flex-col overflow-hidden rounded-xl border border-border bg-background shadow-sm',
          className,
        )}
        {...props}
      />
...
function CodeHeader({
  ...
  return (
    <div
        className={cn(
          'flex h-10 w-full shrink-0 items-center gap-x-2 border-b border-border/70 bg-muted/55 px-4 text-sm text-muted-foreground',
        className,
      )}
      {...props}
    >
