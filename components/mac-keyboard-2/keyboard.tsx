...
const KeyboardProvider = ({
  children,
  enableSound = false,
  soundUrl = "https://sahilcodex.vercel.app/sounds/sound.ogg",
  containerRef,
}: {
  ...
  const playSoundDown = useCallback(
    (keyCode: string) => {
      ...
      // The visual keyboard includes controls such as Touch ID that have no
      // physical keyboard event code in the source sprite. Use a normal key
      // sample for those controls so every rendered button produces feedback.
      const soundDef = SOUND_DEFINES_DOWN[keyCode] ?? SOUND_DEFINES_DOWN.KeyA;
  ...
  const playSoundUp = useCallback(
    (keyCode: string) => {
      ...
      const soundDef = SOUND_DEFINES_UP[keyCode] ?? SOUND_DEFINES_UP.KeyA;
